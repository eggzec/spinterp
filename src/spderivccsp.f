C     *******************************************************************
C
C       SPDERIV_CC_SP - CC gradient, sparse index structure.
C
C       License:
...
C     *******************************************************************
      ...
      SUBROUTINE SPDERIV_CC_SP(D, Z, NZ, Y, NINTERP,
     &    INDICESNDIIMS, NSUBGRIDS,
     &    INDICESDIMS, INDICESLEVS, INDICESADDR, NADDR,
     &    SUBGRIDPOINTS, IP, IPDER)
C     *******************************************************************
C
C       SPDERIV_CC_SP computes interpolated values and gradient for the
C       CC sparse grid interpolant using the sparse index structure.
C
C       Parameters:
C
...
C                 Gradient for each active dim
                  DO DID = 1, NDIMS
                        TEMP = 1.0D+00
                        DO L2 = 1, NDIMS
                              IF (L2 .EQ. DID) THEN

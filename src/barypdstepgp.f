C     *******************************************************************
C
C       BARY_PD_STEP_GP - Barycentric d-dim Lagrange interpolation step,
C                         Gauss-Patterson grid.
C
C       License:
...
C     *******************************************************************
      ...
      SUBROUTINE BARY_PD_STEP_GP(Z, NZ, ALLNX, DIMS, D,
     &                            X, NX, Y, NINTERP, DYFULL,
     &                            W, IP)
C     *******************************************************************
C
C       BARY_PD_STEP_GP evaluates the multi-dimensional polynomial
C       interpolant using barycentric Lagrange interpolation on the
C       Gauss-Patterson grid.  Includes precomputed barycentric
C       weights W in addition to node locations X.
C
C       Parameters:
C
...
C         Input,  DOUBLE PRECISION W(NX)       - barycentric weights
...
C     *******************************************************************
      ...
      INTEGER LOK, UPK, KMID
      DOUBLE PRECISION A(16384)
      DOUBLE PRECISION C1(50), C2(50), S, FRAC, T, XK
...
C     NNA(k) = (ALLNX(k) + 1) / 2  (new GP nodes per dim)
      DO L = 1, D
            NNA(L) = (ALLNX(L) + 1) / 2
      END DO
      ...
      DO K = 1, NINTERP
            ...
            DO L = 1, D
                  ...
                  ELSE
C                       Binary search for t in the node array
                        IF (T .GE. 0.0D+00 .AND. T .LE. 1.0D+00) THEN
                              LOK = XID + 1
                              UPK = XID + ACTNX
                              KMID = (LOK + UPK) / 2
                              XK = X(KMID)
 70                           IF (LOK + 1 .LT. UPK) THEN
                                    IF (XK .GT. T) THEN
                                          UPK = KMID
                                    ELSE
                                          LOK = KMID
                              ...
                              IF (T .EQ. X(LOK)) THEN
                                    K2 = LOK
                              ELSE IF (T .EQ. X(UPK)) THEN
                                    K2 = UPK
                              ELSE
                                    K2 = 0
                              END IF
                              IF (K2 .GT. 0) THEN
                                    IF (MOD(K2-XID, 2) .EQ. 0) THEN
                                          ISZERONODE = 1
                                          GOTO 50
                                    ELSE
                                          ID(L) = (K2-XID+1)/2

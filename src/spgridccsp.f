C     *******************************************************************
C
C       SPGRID_CC_SP - CC grid points, sparse index structure.
C
C       License:
...
C     *******************************************************************
      ...
      SUBROUTINE SPGRID_CC_SP(INDICESNDIIMS, NSUBGRIDS,
     &    INDICESDIMS, INDICESLEVS, INDICESADDR, NADDR,
     &    SUBGRIDPOINTS, D, X, TOTALPOINTS, FROMINDEX, TOINDEX)
C     *******************************************************************
C
C       SPGRID_CC_SP computes CC sparse grid points using sparse index
C       structure. For each subgrid in FROMINDEX..TOINDEX, builds the
C       tensor product of 1D CC nodes and fills X.
C
...
C     Initialize all coords to 0.5 (inactive dims default)
      DO K = 1, D
            DO I = 1, TOTALPOINTS
                  X(I, K) = 0.5D+00
            END DO
...
C     Compute starting IDX: sum of points for subgrids 1..FROMINDEX-1
      IDX = 1
      DO CI = 1, FROMINDEX - 1
      ...
      DO CI = FROMINDEX, TOINDEX
            ...
            IF (NDIMS .EQ. 0) THEN
C                 No active dims: single constant point at 0.5 everywhere
                  IDX = IDX + NPTS
                  GOTO 200
            END IF
